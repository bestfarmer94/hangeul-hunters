...
import com.example.hangeulhunters.infrastructure.service.gcs.GcsService;
...
public class FileService {
    ...
    public PresignedUrlDto generatePresignedUrl(PresignedUrlRequest request) {
        return gcsService.generatePresignedUrl(request.getFileType(), request.getFileExtension());
    }
    ...
    public String saveImageUrl(ImageType imageType, String imageUrl) {
        return gcsService.moveFile(imageType.getPath(), imageUrl);
    }
    ...
    public String uploadTempFile(byte[] data, String fileExtension) {
        return gcsService.uploadTempFile(data, fileExtension);
    }
    ...
    public String saveAudioUrl(AudioType audioType, String audioUrl) {
        return gcsService.moveFile(audioType.getPath(), audioUrl);
    }

    public List<FileDto> saveFiles(Long userId, FileObjectType objectType, Long objectId,
        ...
        if (fileRequests != null && !fileRequests.isEmpty()) {
            for (FileRequest fileRequest : fileRequests) {
                // 파일명 추출 (URL의 마지막 부분)
                // String fileName =
                // fileRequest.getFileUrl().substring(fileRequest.getFileUrl().lastIndexOf('/')
                // + 1);
